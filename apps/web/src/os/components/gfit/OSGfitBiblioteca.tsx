...
import { card2, input, btn, btnGhost, pill, chip, overlay, sheet, sheetBody, sheetHandle, sel } from './estilos';
...
export default function OSGfitBiblioteca() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {loading && !ejercicios.length ? <p style={{ fontSize: 13, color: 'var(--os-muted)' }}>Cargando...</p> :
        !ejercicios.length ? <p style={{ fontSize: 13, color: 'var(--os-muted)' }}>Sin resultados.</p> :
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
            {ejercicios.map((e) => (
              <button key={e.id} onClick={() => setDetalle(e)} style={{
                ...
                <p style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--os-text)', margin: 0, lineHeight: 1.25 }}>{nombreEjercicio(e)}</p>
                <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                  {chipsGrupo(e).slice(0, 2).map((g) => <span key={g} style={{ ...chip, fontSize: 9.5 }}>{g}</span>)}
                </div>
              </button>
            ))}
          </div>
...
function PanelChips({ items, valor, onElegir }: { items: TaxonomiaFila[]; valor: string; onElegir: (v: string) => void }) {
  return (
    <div style={{ ...card2, display: 'flex', gap: 6, flexWrap: 'wrap', maxHeight: 200, overflowY: 'auto' }}>
      ...
      {!items.length && <p style={{ fontSize: 12, color: 'var(--os-muted)', margin: 0 }}>Sin opciones.</p>}
    </div>
  );
}
  ...
  return (
    <div style={overlay} onClick={onCerrar}>
      <div style={sheet} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ padding: '0 1rem 0.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <p style={{ fontSize: 15, fontWeight: 700, color: 'var(--os-text)', margin: 0 }}>{nombreEjercicio(ejercicio)}</p>
        ...
        <div style={sheetBody}>
          ...
          {(ejercicio.instrucciones_en ?? []).length > 0 && (
            <ol style={{ fontSize: 12.5, color: 'var(--os-text-2)', lineHeight: 1.7, paddingLeft: 18, margin: '0 0 12px' }}>
...
function PickerAgregar({ ejercicio, onListo }: { ejercicio: Ejercicio; onListo: () => void }) {
  ...
  if (hecho) return <p style={{ fontSize: 13, color: 'var(--os-champagne)', fontWeight: 700, textAlign: 'center' }}>Agregado ✓</p>;
